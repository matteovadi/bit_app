anticipo_fun = function(selezione){
  if(!is.null(selezione)){
    ...
    if(selezione == nrow(scadenziario_bn)){
      if(scadenziario_bn$Macchina[selezione] == "6250"){
        if(scadenziario_bn$Formato[selezione] == "25x35" | scadenziario_bn$Formato[selezione] == "25X35"){
          residuo6250 = as.numeric(storia.residuo6250.bn[selezione,]) + (as.numeric(scadenziario_bn$Click[selezione])/9400)
        } else {
          residuo6250 = as.numeric(storia.residuo6250.bn[selezione,]) + (as.numeric(scadenziario_bn$Click[selezione])/7400)
        }
      } else {
        if(scadenziario_bn$Formato[selezione] == "25x35" | scadenziario_bn$Formato[selezione] == "25X35"){
          residuo6180 = as.numeric(storia.residuo6180.bn[selezione,]) + (as.numeric(scadenziario_bn$Click[selezione])/6500)
        } else {
          residuo6180 = as.numeric(storia.residuo6180.bn[selezione,]) + (as.numeric(scadenziario_bn$Click[selezione])/5200)
        }
      }
      duedate6180_bn = as.Date(storia.date6180.bn[(selezione),])
      duedate6250_bn = as.Date(storia.date6250.bn[(selezione),])
      scadenziario_bn[selezione,] = NA
      scadenziario_bn = na.omit(scadenziario_bn)
      storia.date6180.bn[selezione,] = NA
      storia.date6180.bn = na.omit(storia.date6180.bn)
      storia.date6250.bn[selezione,] = NA
      storia.date6250.bn = na.omit(storia.date6250.bn)
      storia.residuo6180.bn[selezione,] = NA
      storia.residuo6180.bn = na.omit(storia.residuo6180.bn)
      storia.residuo6250.bn[selezione,] = NA
      storia.residuo6250.bn = na.omit(storia.residuo6250.bn)
      ordinare[selezione,] = NA
      ...
      for (y in (selezione+1):nrow(scadenziario_bn)){
      ...
      for(t in (selezione+1):nrow(storia.date6180.bn) ){
    ...
    showNotification("Attenzione! Non hai selezionato la commessa che hai completato in anticipo!", type = "error")
    return()
  }
}
